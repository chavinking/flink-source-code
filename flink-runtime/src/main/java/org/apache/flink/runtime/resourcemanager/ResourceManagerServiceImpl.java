...
//        leader选举启动
        ...
        handleLeaderEventExecutor.execute(
                () -> {
                    synchronized (lock) {
                        ...
                        try {
                            startNewLeaderResourceManager(newLeaderSessionID); // 选举新leader
                        } catch (Throwable t) {
    ...
    private void startNewLeaderResourceManager(UUID newLeaderSessionID) throws Exception {
        ...
        // 创建rm服务实例，这里进入了resourcemanager服务创建过程
        ...
        previousResourceManagerTerminationFuture
                .thenComposeAsync(
                        (ignore) -> {
                            synchronized (lock) {
                                return startResourceManagerIfIsLeader(newLeaderResourceManager);// 启动rm服务
                            }
                        },
